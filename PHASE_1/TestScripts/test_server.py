...
from ..API_SourceCode.api.server import app
from ..API_SourceCode.api.backend import set_test_mode

client = TestClient(app)

def test_article1():
    set_test_mode()
    response = client.get("/api/v1/articles", params = {"start_date": "2021-01-01T00:00:00", "end_date":"2022-03-15T00:00:00", "key_terms":"ebola,listeria", "location":"Florida"})
    ...
    'log': {'access_time': datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'data_source': 'https://www.cdc.gov/outbreaks/index.html',
            'team_name': 'The Implementers'},
  }

def test_article2():
  response = client.get("/api/v1/articles", params = {"start_date": "2021-08-08T19:37:12", "end_date":"2022-03-15T00:00:00", "key_terms":"ebola", "location":"North Kivu Province"})
  assert response.status_code == 200
  assert response.json() == {
  "articles": [
    {
      ...
      "reports": [
        {
          ...
          "syndromes": [],
          "event_date": "2021-08-08 19:37:12",
          "locations": [
            {
...
def test_article3():
  response = client.get("/api/v1/articles", params = {"start_date": "2018-12-31T00:00:00", "end_date":"2022-03-15T00:00:00", "key_terms":"measles", "location":"Clark County, Washington"})
  assert response.status_code == 200
  assert response.json() == {
  "articles": [
    {
        ...
        "reports": [{
            "diseases": ["measles"],
            "syndromes": [],
            "event_date": '2018-12-31 00:00:00to2018-12-31 00:00:00',
            "locations": [{
                "country": "US",
...
def test_article4():
  response = client.get("/api/v1/articles", params = {"start_date": "2021-07-15T00:00:00", "end_date":"2022-03-15T00:00:00", "key_terms":"bacterial meningitis", "location":"Clark County, Washington"})
  assert response.status_code == 200
  assert response.json() == {
  "articles": [
    {
        ...
        "reports": [{
            ...
            "event_date": '2021-07-15 00:00:00to2021-07-15 00:00:00',
            "locations": [{
                "country": "US",
                "location": "Clark County, Washington"
            }]
...
def test_invalid_date():
    response = client.get("/api/v1/articles", params = {"start_date": "2022-03-15T00:00:00", "end_date":"2021-01-01T00:00:00", "key_terms":"ebola,listeria", "location":"Florida"})
    assert response.status_code == 400
    assert response.json() == {'detail': [{'loc': ['query', 'start_date', 'end_date'], 'msg': 'Invalid Date Range:End date before start date', 'type': ''}]} 
def test_no_params():
    response = client.get("/api/v1/articles")
    assert response.status_code == 400

def test_wrong_format_date():
    response = client.get("/api/v1/articles", params = {"start_date": "2022-03-15", "end_date":"2021-01-01", "key_terms":"ebola,listeria", "location":"Florida"})
    assert response.status_code == 400
    assert response.json() ==  {'detail': [{'loc': ['query', 'date'], 'msg': 'Incorrect date format. Should be yyyy-MM-ddTHH:mm:ss', 'type': ''}]}

def test_no_matching_article():
    response = client.get("/api/v1/articles", params = {"start_date": "2021-01-01T00:00:00", "end_date":"2022-03-15T00:00:00", "key_terms":"hepatitisa", "location":"Australia"})
